...
- For staged calculation enhancements, land reusable domain primitives in `src/mealplan/domain/services.py` and re-export them via `mealplan.domain.__init__` before later stories thread that logic through application orchestration.
...
67. For golden snapshot comparisons, reuse `tests/golden/helpers.py` hybrid assertions: enforce strict key/list ordering and string fields, and apply only `NUMERIC_TOLERANCE_ABS = 0.01` to approved energy/macro numeric fields (`TDEE`, `training_kcal`, `protein_g`, `carbs_g`, `fat_g`, `total_kcal`, and `meals[*].{protein_g,carbs_g,fat_g,kcal}`).
...
107. For each new top-level UI page in `web/ui_server.py`, update all four together: `_PAGE_CONTENT` entry, `_render_app_shell(...)` active-nav `aria-current` substitution, `do_GET` route handling, and shell assertions in `tests/unit/test_web_ui_server.py` so navigation/routing and rendered markers stay synchronized.
