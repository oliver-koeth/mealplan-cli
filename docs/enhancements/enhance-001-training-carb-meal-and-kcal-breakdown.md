...
- Ensure meal-level kcal totals reconcile to the day-level kcal total (`TDEE`).
...
- Add a reconciliation step that adjusts evening snack displayed kcal so summed meal kcal equals total day kcal (`TDEE`) with small rounding corrections.
...
- Small rounding differences are expected and should be corrected only via evening snack displayed-kcal adjustment so final summed meal kcal matches top-level `TDEE`.
...
- Sum of displayed meal kcal equals top-level displayed `TDEE`, with minor rounding delta absorbed by evening snack displayed kcal only.
- Automated tests cover:
  ...
  - Kcal reconciliation to exact `TDEE`.
